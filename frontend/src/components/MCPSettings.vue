  ...
  <div class="card">
    ...
    <div style="margin-bottom: 1.5rem;">
      ...
      <div style="display: flex; gap: 0.5rem; align-items: center;">
        ...
        />
        <button class="btn btn-secondary" @click="copyPrompt" style="padding: 0.5rem 1rem; white-space: nowrap;">Copy</button>
      </div>
    </div>

    <div style="margin-bottom: 1.5rem;">
      <label style="display: block; font-size: 0.875rem; margin-bottom: 0.5rem;">One-Time Password</label>
      <button class="btn" @click="generateOTP" :disabled="otpLoading" style="width: 100%;">
        {{ otpLoading ? 'Generating...' : 'Generate OTP' }}
      </button>
      <div v-if="otpCode" style="margin-top: 0.75rem; padding: 0.75rem; background: var(--bg-secondary); border-radius: 0.25rem;">
        <div style="font-family: monospace; font-size: 1.25rem; font-weight: bold; letter-spacing: 0.25em; text-align: center; margin-bottom: 0.5rem;">{{ otpCode }}</div>
        <div style="font-size: 0.75rem; color: var(--text-secondary);">Expires in {{ otpTimeLeft }}s</div>
      </div>
    </div>

    <div>
      <label style="display: block; font-size: 0.875rem; margin-bottom: 0.75rem;">Authenticated Agents</label>
      <div v-if="agents.length === 0" style="padding: 1rem; text-align: center; color: var(--text-secondary); font-size: 0.875rem;">
        No agents connected
      </div>
      <div v-else style="display: flex; flex-direction: column; gap: 0.75rem;">
        <div v-for="agent in agents" :key="agent.id" style="padding: 0.75rem; background: var(--bg-secondary); border-radius: 0.25rem; display: flex; justify-content: space-between; align-items: center;">
          <div style="flex: 1;">
            <div style="font-weight: 500; font-size: 0.875rem;">{{ agent.name }}</div>
            <div style="font-size: 0.75rem; color: var(--text-secondary); margin-top: 0.25rem;">
              {{ agent.origin }} · {{ formatDate(agent.created_at) }}
            </div>
          </div>
          <button class="btn btn-secondary" @click="revokeAgent(agent.id)" style="padding: 0.5rem 1rem; white-space: nowrap;">Revoke</button>
        </div>
      </div>
    </div>
  </div>
