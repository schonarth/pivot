...
describe('AssetAnalysisTab', () => {
  ...
  it('renders split chart panels and keeps the price pane logarithmic', async () => {
    ...
    expect(mainChartOptions.yaxis.min).toBe(50)
    expect(mainChartOptions.yaxis.max).toBe(200)
    expect(mainChartOptions.yaxis.tickAmount).toBeGreaterThanOrEqual(3)
    ...
    expect(oscillatorChartOptions.colors).toEqual(['#22c55e', '#ef4444', '#3b82f6'])
    expect(oscillatorChartOptions.stroke.width).toEqual([0, 0, 2])
    expect(oscillatorChartOptions.series[2].zIndex).toBe(10)
    ...
    expect(latestMainOptions.colors).toEqual(['#16a34a', '#f59e0b', '#dc2626'])
  })
})
